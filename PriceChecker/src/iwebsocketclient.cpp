...
void IWebsocketClient::setQrCode(const QString &qr, const QString &doc_ref, const QString &row_ref, tree_model::IQMLTreeItemModel *model)
{
    ...
    if(row.isValid()){
        ...
        if(count > 0){
            emit displayError("Ошибка", "QR Код уже подобран к другой позиции номенклатуры!");
            return;
        }

        rs.clear();

        str = builder::query_builder().row_count().from(arcirk::enum_synonym(tables::tbDocumentsMarkedTables)).where(json{
                                                                                                                         {"document_ref", doc_ref.toStdString()}
                                                                                                                     }, true).prepare();
        rs.exec(str.data());
        while (rs.next()) {
        ...
        count++;
        emit model->rowCountChanged(count);
        emit qrCodeChanged(row_ref, count);
    }
}
...
void IWebsocketClient::removeRow(const QString &ref, IQMLTreeItemModel *model)
{
    ...
    if(index.isValid()){
        ...
        if(model->currentObjectName() == QString::fromStdString(arcirk::enum_synonym(tables::tbDocumentsMarkedTables))){
            QString str = builder::query_builder().row_count().from(arcirk::enum_synonym(tables::tbDocumentsMarkedTables)).where(json{
                                                                                                                             {"document_ref", parent}
                                                                                                                         }, true).prepare().c_str();
            rs.clear();
            rs.exec(str);
            ...
            emit model->rowCountChanged(count);
            auto item = object["parent"].get<std::string>();
            emit qrCodeChanged(item.c_str(), count);
...
void IWebsocketClient::verifyUnit()
{
...
//    db.setDatabaseName(ui->lineEdit->text());
    ...
    if(sqlDatabase.open()){
        ...
        QSqlQuery rc("select * from Nomenclature where length(Nomenclature.unit) > 4");
        while (rc.next()) {
            m_vec.append(rc.value("ref").toString());
        }

        if(m_vec.size() == 0)
            return;

        QString script = "Попытка\n"
                        "мСсылкиТекст = СтрРазделить(СтруктураПараметров.links, \",\");\n"
                        "мСсылки = Новый Массив;\n"
                        "Для Каждого Элемент Из мСсылкиТекст Цикл\n"
                        "мСсылки.Добавить(Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(Элемент)));\n"
                        "КонецЦикла;\n"
                        "Запрос = Новый Запрос;\n"
                        ...
                        "Запрос.УстановитьПараметр(\"Ссылки\", мСсылки);\n"
                        "Результат = Запрос.Выполнить().Выгрузить();\n"
                        ""
                        "мРезультат = арс_РаботаСJSON.СтрокаВBase64(арс_РаботаСJSON.ТаблицаЗначенийВJson(Результат, Истина));\n"
                        "СтруктураПараметров.Вставить(\"result\", Новый Структура(\"table\", мРезультат));\n"
                        "Исключение\n"
                        "СтруктураПараметров.Вставить(\"result\", Новый Структура(\"error\", ОписаниеОшибки()));\n"
                        "КонецПопытки;";

        QJsonObject param_script = {
            {"command", "ExecuteScript"},
            {"param", QJsonObject({
                             {"script", QString(script.toUtf8().toBase64())},
                             {"links", m_vec.join(",")},
        ...
        QNetworkRequest request(QUrl("http://192.168.10.9/trade/hs/http_trade/info"));
        request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
        ...
        httpService.post(request, QJsonDocument(param_script).toJson());
        loop.exec();

        if(httpStatus != 200){
            qDebug() << "Ошибка веб запроса!";
        }
        if(httpData.isEmpty())
            return;

        if(httpData == "error"){
            qDebug() << "Ошибка при выполении скрипта!";
            return;
        }

        auto doc = QJsonDocument::fromJson(httpData).object();
        auto res = QJsonDocument::fromJson(QByteArray::fromBase64(doc.value("table").toString().toUtf8())).object();
        auto rows = res.value("Rows").toArray();

        m_vec.clear();
        foreach (const auto itr, rows) {
            auto row = itr.toObject();
            QString ref = row.value("ref").toString();
            QString unit = row.value("unit").toString();
            m_vec.append(QString("update Nomenclature set unit = '%1' where ref = '%2'").arg(unit,ref));
        }

        sqlDatabase.transaction();
        foreach (auto itr, m_vec) {
            sqlDatabase.exec(itr);
        }
        sqlDatabase.commit();

        qDebug() << "Обработка успешно заверешна!";
    }else
        qDebug() << "База данных не открыта!";

}

void IWebsocketClient::checkUpdate()
{
    if(!isStarted()){
        emit displayError("Ошибка", "Отсутствует подключение к серверу!");
        return;
    }

    auto app_version = version_app();
    auto ver = pre::json::to_json(app_version);

    nlohmann::json param{
        {"current_version", ver},
        {"extension", "apk"},
        {"release_dir", "html/files"}
    };
    auto param_ = http_param();
    param_.command = enum_synonym(arcirk::server::server_commands::CheckForUpdates);
    param_.param = QByteArray(param.dump().data()).toBase64().toStdString();
    ...
    httpService.post(request, QByteArray::fromStdString(pre::json::to_json(param_).dump()));
    loop.exec();

    if(httpStatus != 200){
        emit displayError("Ошибка", "Ошибка на сервере обновлений!");
        return;
    }
    ...
    if(msg.result.empty()){
        emit notify("Обновлений не найдено!");
        return;
    }

    if(msg.message == "OK" && !msg.result.empty()){
        //qDebug() << "OK";
        try {
            auto s = QByteArray::fromBase64(msg.result.data()).toStdString();
            qDebug() << qPrintable(s.c_str());
            auto new_version = pre::json::from_json<arcirk::client::version_application>(nlohmann::json::parse(s)["new_version"]);
            int n_ver = (new_version.major * 1000) + (new_version.major * 100) + (app_version.path*10);
            int ver = (app_version.major * 1000) + (app_version.major * 100) + (app_version.path*10);

            emit verifyVersion(n_ver > ver, new_version.major, new_version.minor, new_version.path);
        ...
        emit notify("Новых версий не найдено!");

    httpService.clearConnectionCache();
}

void IWebsocketClient::updateApplication(int major, int minor, int path)
{
    qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << __FUNCTION__;

    QString file_name = QString("files/checker_v%1_%2_%3.apk").arg(QString::number(major), QString::number(minor), QString::number(path) );
    ...
    loop.connect(&httpService, &QNetworkAccessManager::finished, finished);
    QString protocol = wsSettings->isSSL() ? "https://" : "http://";
    QUrl url(protocol + wsSettings->host() + ":" + QString::number(wsSettings->port()) + "/" + file_name);
    QNetworkRequest request(url);
    ...
    if(httpStatus != 200){
        emit displayError("updateApplication", "Ошибка получения файла обновления!");
        return;
    }
    ...
    connect(&thread, &QThread::started, &context, [&]() {
        try {
            if(!httpData.isNull())
            {
                QString home_path = QStandardPaths::writableLocation(QStandardPaths::DownloadLocation);
                home_path += QString("/checker_v%1_%2_%3.apk").arg(QString::number(major), QString::number(minor), QString::number(path));
                QFile f(home_path);
...
#ifdef Q_OS_ANDROID
                emit dowhloadUpdateFileComplete(home_path);
                //emit dowhloadUpdateFileComplete("files/Download/checker_v1_1_1.apk");
#endif
            }

        } catch (const std::exception& e) {
            qCritical() << QDateTime::currentDateTime().toString("hh:mm:ss") << __FUNCTION__ << e.what();
            emit displayError("updateApplication", "Ошибка запуска обновления.");
        }
        loop.quit();
    });
    thread.start();
...
void IWebsocketClient::updateDeviceName(const QString &name)
{
    qDebug() << QDateTime::currentDateTime().toString("hh:mm:ss") << __FUNCTION__;
    ...
    auto query = query_builder();
    query.use(json{{"first", name.toStdString()}});

    auto param = json{
        {"table_name", "update_device_info"}
    };

    std::string query_param = QByteArray::fromStdString(param.dump()).toBase64().toStdString();
    send_command(arcirk::server::server_commands::ExecuteSqlQuery, {
                                                                        {"query_text", query.update(enum_synonym(database::tables::tbDevices), true).where(json{{"ref", wsSettings->deviceId().toStdString()}}, true).prepare()},
                                                                        {"query_param", query_param}
                                                                   });
}
...
void IWebsocketClient::documentTableResetData(const QString &ref, IQMLTreeItemModel *model)
{
    using namespace arcirk::database;
