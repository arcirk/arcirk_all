...
//        }
    ...
    void Settings::read_private_conf(){
        ...
        if(QFile::exists(fileName)){
            ...
            client_conf = secure_serialization<client::client_private_config>(m_text);

            m_device_name = client_conf.deviceName.c_str();
        }
    }
    ...
    QString Settings::httpUrl() const
    {

        return QString("http://%1:%2").arg(conf.ServerHost.c_str(), QString::number(conf.ServerPort));
    ...
    void Settings::setDeviceName(const QString &device_name)
    {
        m_device_name = device_name;
        client_conf.deviceName = m_device_name.toStdString();
    }
