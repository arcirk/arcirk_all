...
public:
    ...
    Q_INVOKABLE void documentTableResetData(const QString& ref, IQMLTreeItemModel* model);
    ...
    Q_INVOKABLE void verifyUnit();

    Q_INVOKABLE void checkUpdate();
    Q_INVOKABLE void updateApplication(int major, int minor, int path);

    Q_INVOKABLE void updateDeviceName(const QString& name);

    Q_INVOKABLE QString version() {
        auto ver = version_app();

        return QString("%1.%2.%3").arg(QString::number(ver.major), QString::number(ver.minor), QString::number(ver.path));
    }
...
private:
    ...
    arcirk::client::version_application version_app(){

        auto app_version = arcirk::client::version_application();
        app_version.major = 1;
        app_version.minor = 2;
        app_version.path = 1;
        return app_version;
...
signals:
    ...
    void qrCodeChanged(const QString& documentRef, int total);

    void verifyVersion(bool result, int major, int minor, int path);
    void dowhloadUpdateFileComplete(const QString& file_path);
};
