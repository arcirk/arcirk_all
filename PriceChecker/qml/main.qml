...
ApplicationWindow {
    ...
    DoQueryBox{
        ...
        visible: false
        onAccept: {
            if(queryDialog.command === "updateApplication"){
                let ver = queryDialog.param;
                footerRow.visible = true
                wsClient.updateApplication(ver["major"], ver["minor"], ver["path"])
            }
        }
        onCancel: {
        }
    ...
    property WebSocketClient wsClient: WebSocketClient{
        ...
        onQrCodeChanged: function(docRef, total){
            if(stackView.currentItem == pageMarkedItems)
                pageDocument.resetItem(docRef)
        }

        onEndAsyncSynchronize: {
            console.debug("end sinchronize")
            //wsClient.poolClear()
        }
        onVerifyVersion: function(result, ma, mi, pt){
            if(result === true){
                queryDialog.text = "Обновленная версия приложения доступна для скачивания. Установить?"
                queryDialog.param = {
                    major: ma,
                    minor: mi,
                    path: pt
                }
                queryDialog.command = "updateApplication"
                queryDialog.visible = true
            }else{
                popupMessage.addNotify("message", "Новых версий не найдено");
                popupMessage.showNotify();
            }
        }
        onDowhloadUpdateFileComplete: function(file_path){
            qtAndroidService.startUpdate(file_path)
...
//        }
    ...
    PageMarkedItems{
        ...
        onErrorMessahge: function(message){
            popupMessage.addNotify("error", message);
            popupMessage.showNotify();
        }
    }
