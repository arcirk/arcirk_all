#include "querybuilderforeginkeydialog.h"
#include "ui_querybuilderforeginkeydialog.h"
    ...
    ui(new Ui::QueryBuilderForeginKeyDialog)
{
    ...
    ui->cmbDeferable->setModel(new QStringListModel(m_deferable, this));
...
void QueryBuilderForeginKeyDialog::set_database_structure(ITree<ibase_object_structure> *structurte){

    m_structurte = structurte;
    if(!m_structurte)
        return;

    auto parent = m_structurte->find(m_structurte->column_index("object_type"), "tablesRoot");
    if(parent.isValid()){
        auto tbl = m_structurte->array(parent);
...
void QueryBuilderForeginKeyDialog::set_object(const sql_foreign_key &object)
{
    ...
    ui->cmbDeferable->setCurrentIndex(m_deferable.indexOf(object.deferrable.c_str()));
...
void QueryBuilderForeginKeyDialog::onCmbTablesCurrentTextChanged(const QString &arg1)
{
    ...
    auto parent = m_structurte->find(m_structurte->column_index("name"), arg1);
    if(parent.isValid()){
        auto fld = m_structurte->array(parent);
