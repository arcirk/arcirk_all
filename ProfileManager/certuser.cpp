...
CertUser::CertUser(QObject *parent)
    : QObject{parent}
{
    m_role = arcirk::database::roles_INVALID;
}
...
void CertUser::set_database_cache(const json &data)
{
    data_ = data;
    cert_user_.cache = data.dump();
}
...
void CertUser::read_database_cache(const QUrl &ws, const QString& token)
{
    ...
    try {
        if(resp.is_object()){
            ...
            if(rows.size() != 0){
                cert_user_ = arcirk::secure_serialization<arcirk::database::cert_users>(rows[0]);
                data_ = json::parse(cert_user_.cache);
            }
        }
    } catch (const std::exception& e) {
        data_ = json::object();
