...
void DialogMstsc::formControl()
{
    ...
    connect(ui->btnSelectHost, &QToolButton::clicked, this, &DialogMstsc::onBtnSelectHostClicked);
...
void DialogMstsc::onSelectHost(const json &hosts)
{
    ...
    auto columns = hosts.value("columns", json::array());
    auto root = json::object();
    for (auto it = columns.begin(); it != columns.end(); ++it) {
        root[*it] = "";
    }

    auto model = new TreeItemModel(root, this);
    ...
    for (int i = 0; i < model->rowCount(QModelIndex()); ++i) {
        ...
        if(type == devDesktop){
            model->set_row_image(index, QIcon(":/img/desktop.png"));
        }else if(type == devServer){
            model->set_row_image(index, QIcon(":/img/server.png"));
        }
    }
...
void DialogMstsc::onBtnSelectHostClicked()
{
    emit selectHost();
}
