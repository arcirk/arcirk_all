...
void TreeItemModel::fetchMore(const QModelIndex &parent)
{
    ...
    if(!parentInfo->mapped()){
        if(m_conf->type_connection() != root_tree_conf::serverDirectorias)
            load_from_database(arcirk::tree::quuid_to_string(parentInfo->ref()));
        else{
...
void TreeItemModel::load_from_database(const QString& parent)
{
        ...
        if(http_result.is_object()){
            ...
            if(arr.size() > 0){
                for (auto itr = arr.begin(); itr != arr.end(); ++itr) {
                    ...
                    auto ref = row.value("parent", NIL_STRING_UUID);
                    if(ref != NIL_STRING_UUID){
                        auto parentIndex = find(tree::to_qt_uuid(ref));
                        add(row, parentIndex);
    ...
    }else if(m_conf->type_connection() == root_tree_conf::serverDirectorias){
...
void TreeItemModel::set_connection(root_tree_conf::typeConnection type, const http::http_conf &value)
{
    ...
    if(type == root_tree_conf::typeConnection::httpConnection){
        load_from_database(NIL_STRING_UUID);
    }else     if(type == root_tree_conf::typeConnection::serverDirectorias){
        load_from_database("");
    }
}
