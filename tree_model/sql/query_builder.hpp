...
BOOST_FUSION_DEFINE_STRUCT(
    ...
    (std::vector<arcirk::database::builder::sql_foreign_key_event>, events)
    ...
    query_builder& create_table(const std::string& name, const std::vector<sql_field> fields){
        ...
        for (auto itr = fields.begin(); itr != fields.end(); ++itr) {
            ...
            if(!itr->foreign_keys.field.empty()){
                ...
                for (auto f_event = itr->foreign_keys.events.begin(); f_event != itr->foreign_keys.events.end(); ++f_event) {
                    sql_foreign_key_event f_e = *f_event;
                    result.append(enum_synonym(f_e));
                    if(f_event != --itr->foreign_keys.events.end())
                        result.append("\n");
                }
            }
            if(itr != --fields.end())
